...
```{r expand formula function, echo=FALSE}
...
+ Training error decreases with increasing numbers of nodes. It also slightly improves, as expected, with the number of obeservations. Training error is also less effected by increasing numbers of noise variables.

+ Test error is much higher with fewer observations or increasing numbers of uncorrelated variables. It's extremely poor with 5 noise variables and only 100 observations.
...
ggplot(data = results.2, aes(x = layer1, y = error + 1, color = as.factor(layer2))) + 
  geom_point(size = 3) + 
  geom_line() +
...
As may be expected, the neural network has a large decrease in error at 7 nodes, equal to the number of input variables. Having 8 nodes seems to pick up extra error, while 12, 16, 24, and 32 don't provide a significan't improvement.

In the two layer model, the best results were from a model with 4 layers in the first node and 16 in the second. It had perfect prediction (values were shifted by 1 for plotting on a $log_{2}$ scale).
