...
```{r SVM linear kernal, cache=TRUE, warning=FALSE}
...
As expected, a linear kernel does not reliably separate the data, as most indicators of location 3 are located in the center of the overall distribution. That said, both very low (overly permissive) and very high (overly restrictive) costs perform worse than the default cost of 1. 

As stated, the problem becomes more "difficult" when the cost is high. This is because a high cost approximates a maximal margin classifier, and the groups aren't completely separated. At cost of 1,000 or 1,000,000, the svm function does not converge on an answer in the maximum number of iterations.

## Use of `tune()` to get best cost

### Code

```{r SVM tune for cost, cache=TRUE, warning=FALSE}
svm.acc <- numeric()
costs <- numeric()
for (i in 1:N) {
...
Regardless of cost, the maximal accuracy appears to be 80%; the minimum is 73%. Simply calling all data points "not location 3" should give 75% accuracy given that only 25% of data points are location 3. Overall performance of the linear kernel is poor.

Within the costs in the range of  0.0625 to 128 ($2^{-4}$ to $2^7$), all costs come up as the best at least once, but the median value is 2 and lower values appear to be more highly represented.
...
The confusion matrix output by `randomForest` is "the confusion matrix of the prediction (based on OOB data)". Out-of-bag data means that not used in the bootstrap process, which gives the (rough) equivalent of multiple training/test splits.

As might be expected, random forest far outperforms SVM with a linear kernal -- 98.6% accuracy to (maximally) 80% accuracy. Making hundreds of less-perfect predictions and relying on the aggregate prediction is a powerful way to overcome error in any one model. The multiple branches of a decision tree can also approximate a non-linear decision boundary.
...
```{r tune.knn, cache=TRUE, warning=FALSE}
knn.acc <- numeric()
kvals <- numeric()
for (i in 1:N) {
  train <- sample(c(TRUE, FALSE), size = nrow(wifiLocDat), replace = TRUE)
...
For K values in the range of 1 - 128, no K value over 16 is observed, suggesting more local classifiers perform better. The median value of K is 3.

The tuned KNN classifiers perform very well, with accuracies in the range of 97.4% to	99.2%. This is comparable to the random forest classifier above and much better than SVM with a linear kernel.
...
```{r SVM gamma, cache=TRUE, warning=FALSE}
for (g in c(0.1, 0.5, 5, 50)){
  ...
  assess.prediction(actual = wifiLocDat$Loc3, 
                    predicted = predict(svm.g.result))
  plot(
    svm.g.result,
    data = wifiLocDat,
    formula = WiFi2 ~ WiFi1
...
As gamma increases, the number of support vectors increases. This is because the model is becoming increasingly local, effectively consisting of the points themselves at higher levels. As gamma increases, the value of the kernel function _decreases_ ($e^{-\gamma|u-v|^2}$), meaning that only extremely close points will have a meaningful effect.

While the accuracy may appear better with high gamma, this is almost certainly overfitting and would perform poorly on novel data.
...
Due to the computational cost, it seems that an iterative process should be used to narrow down the parameter range, with a few simulations at a broad range moving toward a larger number at a narrower range.

I initially ran a small number of replicates ($n = 10$) with `cost = 2 ^ (-3:8)` and `gamma = sort(c(10 ^ (-3:3) / 2, 10 ^ (-3:3)))`.
...
Not many points were observed at a cost of 128 or 256, so they were dropped. Values of gamma in the range of 0.05 to 0.5 were most frequently observed, so those were used for the final simulation

Final parameter range choice is `cost = 2 ^ (1:6)`, `gamma = c(0.05, 0.1, 0.25, 0.5)`

                            
```{r SVM gamma and cost, cache=TRUE, warning=FALSE}
svm.g.acc <- numeric()
costs.g <- numeric()
gammas <- numeric()
for (i in 1:N) {
...
SVM with a radial kernel performs much better than its linear counterpart (accuracy range of 97.1% to	98.9%) and on par with random forest and (tuned) KNN.

The median values of cost and gamma found were 8 and 0.25, respectively. There is likely potential for further refinement by exploring more fine-grained values in this range.
...
```{r SVM polynomial, cache=TRUE, warning=FALSE}
...
for (deg in c(3, 4)){
  for (coef0 in c(0, 0.1, 1)){
    ...
    assess.prediction(actual = wifiLocDat$Loc3, 
                      predicted = predict(svm.p.result))
    plot(
      svm.p.result,
      data = wifiLocDat,
      formula = WiFi2 ~ WiFi1
...
The polynomial kernel performs on par with the radial one (accuracy range 97.3% to 99.1%) and has potential for further improvement due to the limited number of parameters explored in this simulation.

The median values found for cost, degree, and coef0 were 5, 2, and 1, respectively. 

Although polynomials up to 5th order were used, only 2nd, 3rd, and 4th were observed. While intuition may say that a higher-order polynomial would provide a better, more local fit, it's likely that it would overfit the training data and perform poorly on the test data.
